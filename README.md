...
### 1. 格式化
- **功能**: 将 EPUB 文件重新打包为标准的格式。
- **作用**: 修复某些不规范的 EPUB 文件结构，使其能够被阅读器正确识别。
- **注意**: 格式化过程会解压并重新压缩文件，可能会清理部分冗余文件。

### 2. 文件解密 (文件名解密)
- **功能**: 将 EPUB 内部经过混淆的文件名还原为可读的名称。
- **作用**: 用于处理某些对文件名进行了加密混淆的 EPUB 文件，方便后续编辑。

### 3. 文件加密 (文件名加密)
- **功能**: 将 EPUB 内部的文件名进行混淆加密。
- **作用**: 保护 EPUB 内容结构，防止轻易被查看或编辑。

### 4. 字体加密
- **功能**: 对 EPUB 内嵌的字体文件进行加密处理。
- **作用**: 保护字体版权，防止字体文件被轻易提取使用。

### 5. 字体子集化
- **功能**: 分析 EPUB 内容，移除字体文件中未被使用的字符。
- **作用**: 大幅减小字体文件体积，从而减小 EPUB 文件总大小，加快加载速度。

### 6. 图片转 WebP
- **功能**: 将 EPUB 中的 JPG/PNG 图片转换为 WebP 格式。
- **作用**: 在保持画质的前提下，显著减小图片体积。

### 7. WebP 转图片
- **功能**: 将 EPUB 中的 WebP 图片还原为 JPG/PNG 格式。
- **作用**: 兼容不支持 WebP 格式的老旧阅读器（如部分 Kindle）。

### 8. PNG 压缩
- **功能**: 对 EPUB 中的 PNG 图片进行无损压缩。
- **作用**: 减小文件体积。

### 9. 简转繁 / 繁转简
- **功能**: 使用 OpenCC 对 EPUB 内容进行高质量的简繁体中文转换。
- **作用**: 满足不同地区的阅读习惯。

### 10. 正则注释
- **功能**: 使用正则表达式匹配特定的注释标记，并自动转换为 EPUB 标准脚注格式。
- **输入**: 点击后需输入正则表达式（如 `<span ... data-note="(.*?)"></span>`）。
- **效果**: 自动生成上标注音图标，并在文末生成带有回跳链接的脚注列表。
