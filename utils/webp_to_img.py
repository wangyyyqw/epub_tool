...
class WebPToImage:
    ...
    def read_files(self):
        for img_path in self.images:
            ...
            try:
                ...
                img_basename = os.path.basename(img_path)  # xxx.webp 或 xxx.png
                ...
                if image.mode in ("RGBA", "LA") or (
                    ...
                    buffer = BytesIO()
                    image = image.convert("RGB") # 确保转换为RGB，防止调色板模式保存为JPEG出错
    ...
    def replace(self):
        ...
        for meta in root.findall('.//opf:meta[@name="cover"]', ns):
            ...
            if content and content.lower().endswith((".webp", ".png")):
        ...
        for item in root.findall(".//opf:item", ns):
            ...
            if media_type in ("image/webp", "image/png"):
                id = item.get("id")
                href = item.get("href")
                ...
                if href_basename in self.img_dict:
                    ...
                    item.set("id", replace_name) # ID 也更新，虽然不是必须的，但保持一致
                    item.set(
        ...
        for html_path in self.htmls:
            ...
            def replace_match(match):
                # match.group(2) 是路径部分（不含扩展名），match.group(3) 是扩展名
        ...
        for css_path in self.css:
            ...
            def replace_match(match):
                ...
                path_with_ext = match.group(2) + "." + match.group(3)  # 比如 "../Images/cover.webp"
...
if __name__ == "__main__":
    it = WebPToImage("test/demo.epub", "./test/")
    it.read_files()
    it.replace()
    it.close_files()
